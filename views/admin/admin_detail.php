						...
						<div class="card">
							...
							<table class="table mt-3">
								...
								<tbody>
                                    ...
                                    $i = 0;
                                    $total_price = 0;
										foreach ($queryLKDc as $queryLKD) {
                                    $i++;
                                    $total_amount = $queryLKD['total_price']*$queryLKD['total_amt'];
                                    $total_price += $total_amount;

									?>
									<tr>
                                        <td><?= $i ?></td>
										<td><?= $queryLKD['order_id'] ?></td>
										<td><img src="views/admin/img/<?= $queryLKD['product_img'] ?>" class="rounded mx-auto d-block" alt="" width="150" height="150"></td>
                                        <td><?= $queryLKD['product_name'] ?></td>
                                        <td><?= $queryLKD['total_amt'] ?></td>
                                        <td><?= $queryLKD['total_price'] ?>$</td>
                                        <td><?= $total_amount ?>$</td>
										<!-- <td><a href="index.php?controller=order" class="btn btn-warning" tabindex="-1" role="button" aria-disabled="true">OK</a>
										</td> -->
									</tr>
									<?php
									    }
							...
							<a href="index.php?controller=order&action=admin_history" class="btn btn-info" tabindex="-1" role="button" aria-disabled="true">Back</a>
						</div>
					</div>
				</div>
			</div>
